...
var costs_df = null;
var filename_costs_df = null;
var group_costs_df = null;
var filename_group_costs_df = null;
var month_summary_df = null;
var filename_month_summary_df = null;

var filename_final_df = null;
var filename_summary_final_df = null;
var component_labels = null;
var component_summary = null;
var end_graph_labels = null;
var end_graph_summary = null;
var fr_price_monthly = null;
var fr_price_monthly_label = null;
var data_loadprofile = null;
var label_loadprofile = null;
...
function listners() {
    $('#fetch').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (response_json) {
                ...
                costs_df = response_json["costs_df"];
                filename_costs_df = response_json["filename_costs_df"];
                group_costs_df = response_json["group_costs_df"];
                filename_group_costs_df = response_json["filename_group_costs_df"];
                month_summary_df = response_json["month_summary_df"];
                filename_month_summary_df = response_json["filename_month_summary_df"];

                component_labels = response_json["component_labels"];
                component_summary = response_json["component_summary"];
                end_graph_labels = response_json["end_graph_labels"];
                end_graph_summary = response_json["end_graph_summary"];
                fr_price_monthly = response_json["fr_price_monthly"];
                fr_price_monthly_label = response_json["fr_price_monthly_label"];
                data_loadprofile = response_json["data_loadprofile"];
                label_loadprofile = response_json["label_loadprofile"];
                ...
                document.getElementById('formcontrol').classList.remove('hidden');
    ...
    $('#fetch_costs').on('click', function () {
        download_costs_sheet();
    });
    $('#fetch_group_costs').on('click', function () {
        download_group_costs_sheet();
    });
    $('#fetch_month_summary').on('click', function () {
        download_month_summary_sheet();
    });
...
function download_costs_sheet() {
    var zip = new JSZip();

    // Iterate over the CSV strings and add them to the ZIP file
    for (var i = 0; i < costs_df.length; i++) {
        var csvString = costs_df[i];
        var filename = filename_costs_df[i] || `file_${i + 1}.csv`; // Default filenames if not provided
        zip.file(filename, csvString);
    }
    ...
    zip.generateAsync({ type: "blob" })
        .then(function (blob) {
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(blob);
            link.download = "costs_sheets.zip";  // Name of the downloaded ZIP file
            link.click();
            window.URL.revokeObjectURL(link.href);
        });
}
function download_group_costs_sheet() {
    var zip = new JSZip();

    // Iterate over the CSV strings and add them to the ZIP file
    for (var i = 0; i < group_costs_df.length; i++) {
        var csvString = group_costs_df[i];
        var filename = filename_group_costs_df[i] || `file_${i + 1}.csv`; // Default filenames if not provided
        zip.file(filename, csvString);
    }
    ...
    zip.generateAsync({ type: "blob" })
        .then(function (blob) {
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(blob);
            link.download = "group_costs_sheets.zip";  // Name of the downloaded ZIP file
            link.click();
            window.URL.revokeObjectURL(link.href);
        });
}
function download_month_summary_sheet() {
    var zip = new JSZip();

    // Iterate over the CSV strings and add them to the ZIP file
    for (var i = 0; i < month_summary_df.length; i++) {
        var csvString = month_summary_df[i];
        var filename = filename_month_summary_df[i] || `file_${i + 1}.csv`; // Default filenames if not provided
        zip.file(filename, csvString);
    }
    ...
    zip.generateAsync({ type: "blob" })
        .then(function (blob) {
            var link = document.createElement('a');
            link.href = window.URL.createObjectURL(blob);
            link.download = "month_summary_sheets.zip";  // Name of the downloaded ZIP file
            link.click();
            window.URL.revokeObjectURL(link.href);
        });
}
...
function generate_hourly(i) {
    ...
    let fixed = end_graph_summary[i][1];
    var data = [{
        x: fr_price_monthly_label[i],
        y: fr_price_monthly[i],
        type: 'line',
        name: 'Monthly Full Requirements'
    },
    {
        x: fr_price_monthly_label[i],
        y: Array(fr_price_monthly_label[i].length).fill(fixed), // Same average for each interval
        mode: 'lines',
        line: {
            color: 'red',
            width: 2,
...
function generate_usage(i) {
    ...
    {
        x: label_loadprofile[i],
        y: Array(label_loadprofile[i].length).fill(average), // Same average for each interval
        mode: 'lines',
        line: {
            color: 'red',
            width: 2,
...
function populate_summary_table(index) {
    ...
    for (var i = 0; i < component_summary[index].length; i++) {
        ...
        td.textContent = component_labels[index][i].replace('data_', '').replace('_shaped', '');
    ...
    for (var i = 0; i < end_graph_summary[index].length; i++) {
        // Create a single row for the values (since your JSON object is a single record)
        var valueRow = document.createElement("tr");
        valueRow.style.borderTop = "0.5px solid rgb(211, 211, 211)";
        ...
        var td = document.createElement("td");
        td.style.padding = "4px 12px";
        td.textContent = end_graph_labels[index][i];
        valueRow.appendChild(td);
        var td = document.createElement("td");
        td.style.padding = "4px 12px";
        td.textContent = end_graph_summary[index][i].toFixed(4);
        valueRow.appendChild(td);
        // valueRow.onclick = ClickHandler(i)
        tbody.appendChild(valueRow);
    }
